...
public:
	...
	void Add(FString InKey, int64 InValue)
	{
		if (InKey.Len() == 0)
		{
			return;
	...
	void Add(FString InKey, TArray<int64> Data)
	{
		if (InKey.Len() == 0 || Data.Num() == 0)
		{
			return;
	...
	void AddObject(FString FieldName, const TSharedPtr<FJsonObject>& JsonObject)
	{
		if (!Obj)
		{
			Obj = MakeShareable(new FJsonObject);
		}
		Obj->SetObjectField(FieldName, JsonObject);
	}
