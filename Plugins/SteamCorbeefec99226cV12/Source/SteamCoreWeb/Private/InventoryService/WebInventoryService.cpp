...
void UWebInventoryService::AddItem(const FOnSteamCoreWebCallback& Callback, FString Key, int32 AppId, TArray<int32> ItemdefId, FString ItemPropsJson, TArray<int32> ItemQuantity, FString SteamId, bool bNotify, FString RequestId, bool bTradeRestriction)
{
	FOnlineAsyncTaskSteamCoreWebAddItem* Task = new FOnlineAsyncTaskSteamCoreWebAddItem(this, Callback, Key, AppId, ItemdefId, ItemPropsJson, ItemQuantity, SteamId, bNotify, RequestId, bTradeRestriction);
	QueueAsyncTask(Task);
}
...
void UWebInventoryService::ExchangeItem(const FOnSteamCoreWebCallback& Callback, FString Key, int32 AppId, FString SteamId, TArray<int64> materialsItemID, TArray<int32> MaterialsQuantity, int64 OutputItemdefId)
{
...
void UWebInventoryService::ModifyItems(const FOnSteamCoreWebCallback& Callback, FString Key, int32 AppId, FString SteamId, FModifyItemsItem Updates)
{
	FOnlineAsyncTaskSteamCoreWebModifyItems* Task = new FOnlineAsyncTaskSteamCoreWebModifyItems(this, Callback, Key, AppId, SteamId, Updates);
	QueueAsyncTask(Task);
}
