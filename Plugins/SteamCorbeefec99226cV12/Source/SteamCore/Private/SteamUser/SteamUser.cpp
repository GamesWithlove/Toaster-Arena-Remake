...
void UUser::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	OnGetTicketForWebApiResponseCallback.Register(this, &UUser::OnGetTicketForWebApiResponse);
#endif
...
ESteamVoiceResult UUser::DecompressVoice(const TArray<uint8>& CompressedBuffer, int32 DesiredSampleRate, TArray<uint8>& DestBuffer)
{
	...
	if (SteamUser())
	{
		...
		//LogSteamCoreVerbose("Bytes Written: %d, destBuffer: %d", BytesWritten, destBuffer.Num());
	}
#endif
...
FSteamTicketHandle UUser::GetAuthTicketForWebApi(FString Identity)
{
	LogSteamCoreVeryVerbose(""); 
...
#if STEAM_VERSION >= 157
void UUser::OnGetTicketForWebApiResponse(GetTicketForWebApiResponse_t* pParam)
{
	LogSteamCoreVerbose("");
