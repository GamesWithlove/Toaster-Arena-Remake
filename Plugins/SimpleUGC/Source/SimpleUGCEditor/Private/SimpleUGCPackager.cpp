...
void CreateUatTaskWithCallback(const FString& CommandLine, const FText& PlatformDisplayName, const FText& TaskName, const FText& TaskShortName, const FSlateBrush* TaskIcon, TFunction<void(const FString&, double)> Callback)
{
	// Convert the callback to the required type
	TFunction<void(FString, double)> ConvertedCallback = [Callback](FString TaskResult, double TimeSec)
	{
		Callback(TaskResult, TimeSec);
	};

	IUATHelperModule::UatTaskResultCallack ResultCallback = ConvertedCallback;

	IUATHelperModule::Get().CreateUatTask(CommandLine, PlatformDisplayName, TaskName, TaskShortName, TaskIcon, nullptr, ResultCallback, FString());
}
...
void FSimpleUGCPackager::PackagePlugin(TSharedRef<class IPlugin> Plugin, const FString& OutputDirectory)
{
	...
	CreateUatTaskWithCallback(CommandLine, PlatformName, PackagingText, PackagingText, FSimpleUGCEditorStyle::Get().GetBrush(TEXT("SimpleUGCEditor.PackageUGCAction")),
		[ReleaseVersion, PlatformName, FriendlyName, OutputDirectory, StagePath, StagePathPackedMod]
	(const FString& TaskResult, double TimeSec)
		{
