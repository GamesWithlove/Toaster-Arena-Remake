...
{
	// Load up the AssetRegistry, Filter for all assets
	...
	// Create a FTopLevelAssetPath for UBlueprint::StaticClass
	FTopLevelAssetPath BlueprintClassPath(UBlueprint::StaticClass()->GetPathName());
...
bool UUGCRegistry::GetMapsInPackage(FUGCPackage Package, TArray<FName>& Maps)
{
	...
	// Create a FTopLevelAssetPath for UWorld::StaticClass
	FTopLevelAssetPath WorldClassPath(UWorld::StaticClass()->GetPathName());
